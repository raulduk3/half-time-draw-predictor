...
% === TITLE ===
\begin{center}
{\color{accent}\LARGE\bfseries Half-Time Draw Prediction:\\[4pt] Empirical Evidence of a Profitable Betting Signal}\\[12pt]
...
\begin{quote}
\color{dimgray}\small
A two-model system identifies profitable half-time draw bets across 22 European football leagues. Model A reads B365 odds (market view). Model B uses 42 non-odds features (fundamentals view). When the market prices a draw higher than fundamentals suggest, actual draws exceed market expectations. On 30,025 out-of-sample matches (Apr 2022 to Feb 2026), the ``Strong Value'' tier ($\geq$5\% edge) achieves a 48.0\% hit rate against a 40.7\% base rate on 3,340 bets. All 9 half-year periods are profitable. Bootstrap P(profit) = 100.0\% across 10,000 resamples.
\end{quote}
...
\bottomrule
\end{tabular}
\caption{Edge tier backtest (flat bet on B365 FT draw odds, 30,025 matches). Per-match DC/Elo.}
\end{table}

\noindent\textbf{Note:} All tiers show positive ROI because B365 full-time draw odds are structurally generous. The primary evidence of model validity is the \textbf{monotonic hit rate increase} across positive edge tiers (40.7\% $\rightarrow$ 42.1\% $\rightarrow$ 42.5\% $\rightarrow$ 48.0\%).
...
\bottomrule
\end{tabular}
\caption{Strong Value temporal stability. No alpha decay.}
\end{table}
...
\bottomrule
\end{tabular}
\caption{Bootstrap statistics (10,000 resamples).}
\end{table}
...
\begin{enumerate}[leftmargin=1.5em, itemsep=2pt]
  \item \textbf{FT odds proxy.} Backtest uses B365 full-time draw odds. All edge tiers show positive ROI, indicating FT odds are structurally generous. The hit rate monotonic increase is the primary evidence; absolute ROI will differ with real HT odds.
  \item \textbf{No transaction costs.} Bet availability, liquidity constraints, and bookmaker limits are not modeled.
  \item \textbf{Prospective validation nascent.} 1 live bet placed (Feb 2026). Real HT odds collection via The Odds API began Feb 2026. Three months of prospective data with actual HT market prices will resolve the open ROI question.
\end{enumerate}

\vspace{8pt}
\noindent\textbf{Key open question:} What is the ROI with actual HT draw odds? The hit rate signal (48.0\% on Strong Value vs 40.7\% base) is robust. Profitability depends on available HT market prices, which will be established through prospective validation.
